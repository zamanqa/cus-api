...
// cypress-mochawesome-reporter stores data as <body data-raw="HTML-ENCODED-JSON">
// All " inside the JSON value are &quot;, so the attribute is safely
// delimited by the first literal " after data-raw="
function readMochawesomeReport() {
  try {
    ...
    const valEnd   = html.indexOf('"', valStart);   // safe: inner " are &quot;
...
function handleRun(req, res) {
  ...
  let currentFile  = '';
  let specTotal    = 0;    // totals from spec table lines
  let specPasses   = 0;
  let specFailures = 0;
  let lineBuffer   = '';
  ...
  const processLine = (rawLine) => {
    ...
    // "Running: cypress/e2e/01-order-page/orderDetail.cy.js" → file started
    ...
    // Columns: checkmark  filename  duration  tests  passing  failing  pending  skipped
    // Failing/pending/skipped can be '-' when zero — handle both digits and dashes.
  ...
  child.on('close', (code) => {
    ...
    // Scalar counts: prefer mochawesome array length, fall back to spec-done totals
    // so the history card always shows the real counts even when mochawesome fails.
    ...
    try {
      ...
      // Auto-cleanup: keep only the latest 10 reports
      pruneRunHistory(10);
    } catch (e) {
...
function pruneRunHistory(keep) {
  try {
    const files = fs.readdirSync(HISTORY_DIR)
      .filter(f => f.endsWith('.json'))
      .sort((a, b) => b.localeCompare(a)); // newest first (filenames are timestamp-based)
