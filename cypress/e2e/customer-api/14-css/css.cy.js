...
describe('Customer Self Service (CSS) API', () => {
  ...
  // Fetch active consumable + normal subscriptions from DB before each test
  beforeEach(() => {
    css.getActiveConsumableSubscriptionFromDB().then((result) => {
      ...
      cy.log('Status:', sub.status);
      cy.log('Type:', sub.subscription_type);
      cy.log('Frequency:', sub.subscription_frequency);
  ...
  it('Test 1: Fetch subscription deliveries and verify billing_date', () => {
    ...
    cy.log('Fetching deliveries for subscription:', subscriptionId);
  ...
  it('Test 2: Report an issue for a subscription', () => {
    ...
    cy.log('Reporting issue for subscription:', subscriptionId);
    cy.log('Appointment date:', futureDate);
  ...
  it('Test 4: Change subscription frequency', () => {
    ...
    cy.log('New frequency: weekly, interval: 2');

    css.changeSubscriptionFrequency(subscriptionId, 'weekly', 2).then((response) => {
      expect(response.status).to.eq(200);
  ...
  it('Test 5: Perform bundle swap for a subscription', () => {
    ...
    css.getProductVariantIdFromDB().then((result) => {
      ...
      cy.log('Bundle swap for subscription:', subscriptionId);
      cy.log('Product variant ID:', variantId);
      cy.log('Variant title:', result[0].title);
  ...
  it('Test 6: Cancel a subscription', () => {
    ...
    cy.log('Cancelling subscription:', subscriptionId);
    cy.log('Pickup date:', futureDate);
  ...
  it('Test 7: Process buyout for a subscription with stripe payment', () => {
    css.getStripeBuyoutSubscriptionFromDB().then((result) => {
      ...
      cy.log('Processing buyout for subscription:', subscriptionId);
      cy.log('Payment provider:', result[0].payment_provider);
  ...
  it('Test 8: Create order by customer', () => {
    css.getOrderVariantFromDB().then((result) => {
      ...
      cy.log('Creating order with variant_id:', data.variant_id);
      cy.log('Parent order ID:', data.parent_order_id);
      cy.log('Variant title:', data.variant_title);
      cy.log('Subscription start:', futureStart);
      ...
      css.createOrderByCustomer(orderPayload).then((response) => {
        ...
        cy.log('Order creation success:', response.body.success);
        cy.log('Response message:', response.body.message);
