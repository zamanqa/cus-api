...
// Get a valid subscription product variant for creating draft orders
...
// Verify draft order was created in DB by draft_id
export function verifyDraftOrderCreatedQuery(companyId, draftOrderId) {
  return `
    ...
    FROM public.draft_orders
    WHERE company_id = '${companyId}'
      AND draft_id = '${draftOrderId}'
      AND deleted_at IS NULL
  `;
}

// Verify draft order was deleted (soft delete — deleted_at IS NOT NULL)
